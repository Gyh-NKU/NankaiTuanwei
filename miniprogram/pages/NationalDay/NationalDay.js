...
Page({
  ...
  data: {
    ...
    backgrounds: ['cloud://nankaituanwei-j5pm1.6e61-nankaituanwei-j5pm1-1257843133/resources/NationalDay/avatars/NationalDayAvatar-1.png'],
    current: 0,
    total: 7
  ...
  onLoad: function() {
    ...
    for(let i = 0; i < this.data.total ; i++){
      wx.cloud.downloadFile({
        fileID: this.data.backgrounds[0].substr(0,112) + (1+i) + '.png',
        success: res => {
          tempFilePath[i] = res.tempFilePath;
          this.data.backgrounds = tempFilePath;
          if(this.data.backgrounds.length === 7){
            this.setData({
              backgrounds: tempFilePath
            })
          }
        }
      })
  ...
  createAvatar: async function (index) {
    ...
    //等待获取图片尺寸，延迟计算
      ...
      switch(true){
        ...
        case index === 1:
          dWidth = 350; break;
        case index === 2 || index === 3 || index === 4 || index === 6:
          dWidth = 500; break;
        case index === 5:
          dWidth = 600; break;
      }
      ...
      context.drawImage(that.data.backgrounds[index], 0, 0, 600, 600);
      context.draw();
      }, 2000)
